...
=begin SYNOPSIS
    ...
    # lines that go directly to the pure Raku parser.
...
This process B<will> be simplified and encapsulated in the near future, as reformatting Raku code was what this rather extensive module was designed to do.
...
You're most welcome to use the Raku debugger, but what I just do is add a C<say 1;> statement just above the C<@child.append()> method call (and anywhere else I find a C<infix> and C<OPER> hash key hiding, because there are multiple places in the code that match an C<infix> and C<OPER> hash key) and rerun the test.
...
Well, the reason it's called C<NQP> is because it's Not Quite Raku, and like poor Rudolph, Raku won't let NQP objects join in any subclasing or dispatching games, because it's Not Quite Perl. And yes, this leads to quite a bit of frustruation.
...
Now we turn to the rather bewildering array of methods on the C<Raku::WS> class. This profusion of methods is because of two things:
...
=item _roundtrip( Str $perl-code ) returns Raku::Parser::Root

Given a string containing valid Raku code ... well, return that code. This
...
Returns the underlying NQPMatch object. This is what gets passed on to C<build-tree()> and every other important method in this module. It does some minor wizardry to call the Raku reentrant compiler to compile the string you pass it, and return a match object. Please note that it B<has> to compile the string in order to validate things like custom operators, so this step is B<not> optional.

=item build-tree( Mu $parsed )

Build the Raku::Element tree from the NQPMatch object. This is the core, and runs the factory which silly-walks the match tree and returns one or more tokens for every single match entry it finds, and B<more>.

=item consistency-check( Raku::Element $root )
...
my role Debugging {
	...
	method _interrogate-element( Raku::Element $node ) {
		my @problem;
		if $node.WHAT.perl eq 'Raku::Element' {
		...
		if $node.is-twig {
			...
			given $node {
				when Raku::Comment | Raku::String | Raku::Sir-Not-Appearing-In-This-Statement { }
				when Raku::WS | Raku::Newline {
	...
	#constant indent = "\t";
	my constant indent = ' ';
	method _dump-tree( Raku::Element $root,
	...
	method dump-tree( Raku::Element $root,
			  Bool $display-ws = True,
			  Int $depth = 0 ) {
		my $str;
	...
	method dump-term( Raku::Element $term ) {
		...
		if $term.is-leaf {
			...
			# Their children do.
			elsif $term ~~ Raku::Operator::PostCircumfix or
			      $term ~~ Raku::Operator::Circumfix {
			}
			else {
		...
		if $term.^can('from') and not (
				$term ~~ Raku::Document | Raku::Statement
			) {
...
my role Validating {
	...
	method _consistency-check( Raku::Element $node ) {
		...
		if $node.is-twig {
			if $node.child.elems > 1 {
				for $node.child.kv -> $index, $_ {
					...
					next if $index == 0;
					if $node.child.[$index-1] ~~ Raku::WS and
					   $node.child.[$index] ~~ Raku::WS {
	...
	# sigh.
	#
	method consistency-check( Raku::Element $root ) {
...
my class Munge-Phasers {
	...
	method _munge-element( Raku::Element $node ) {
	...
	method _munge-phasers( Raku::Element $node ) {
	...
	# sigh.
	#
	method munge-phasers( Raku::Element $root ) {
		return unless @.munged-BEGIN or @.munged-CHECK;
		self._munge-phasers( $root );
	}
}

class Raku::Parser:ver<0.3.0> {
	also does Debugging;
	also does Testing;
	also does Validating;
	use nqp;

	has $.factory = Raku::Parser::Factory.new;
	has @.munged-BEGIN;
	has @.munged-CHECK;
	...
	method parse( Str $source ) {
		my $*LINEPOSCACHE;
		my $compiler := nqp::getcomp('Raku') // nqp::getcomp('raku');
		my $g := nqp::findmethod(
	...
	method to-string( Raku::Element $tree ) {
